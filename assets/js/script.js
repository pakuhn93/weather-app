...
var geocodeFullAPI = "http://api.openweathermap.org/geo/1.0/direct?q={city name},{state code},{country code}&limit=" + locationLimit + "&appid=" + keyAPI;
var geocodeCityAPI = "http://api.openweathermap.org/geo/1.0/direct?q={city name}," + locationLimit + "&appid=" + keyAPI;
...
var stateNameEl = document.getElementById('state');
var submitBtn = document.getElementById('search-btn'); //grab button element
...
function generateLocation(cityName, stateCode, countryCode){
    ...
    cityGeocode = cityGeocode.replace("{city name}", cityName);
...
//grabs user input when the search button is clicked
